# CRUD vs REST+DDD: measured refactor metrics

Generated by `npm run measure` (`docs/measure-refactor.mjs`).
Compares three points on the refactor's timeline: `crud`
(`81690b1`, the last commit before slice 0 --
full CRUD shape), `json-hypermedia` (`a00cc6f`, an
intermediate branch: the backend is fully hypermedia-driven and
command-based, but elevator-ui is still a Vue SPA talking JSON
through a BFF, with no HTML responses from elevator-api), and
`main` (`d5d01b7`, current tip: no BFF,
elevator-api serves HTML directly, Datastar morphs it in place).
Re-run this script any time any of the three refs moves.

`json-hypermedia`'s own history shows its elevator-api side already
deleted the CRUD scaffolding independently of `main`'s --
so most backend metrics below are expected to already match
`main`, and that convergence is itself reported rather than
hidden. Where the three points actually diverge is the front end:
`json-hypermedia` shows what hypermedia buys *without* also removing
the smart client -- see sections 4 and 10 in particular.

## 1. Lines of code and file counts

| | `crud` | `json-hypermedia` | `main` |
|---|---|---|---|
| elevator-api `main` files / lines | 24 / 1043 | 125 / 5831 | 133 / 6598 |
| elevator-api `test` files / lines | 9 / 970 | 44 / 3036 | 47 / 3336 |
| **Largest file:** | | | |
| File | `ElevatorService.java` | `Elevator.java` | `Elevator.java` |
| Lines | 501 | 414 | 414 |

File count rises with vertical slicing by design -- one directory
per behaviour, each holding its own command/handler/endpoint/tests.
`json-hypermedia` already matches `main` almost exactly here --
its elevator-api side finished the same migration independently.
Section 2 checks whether those smaller files are also simpler.

## 2. Complexity, not just size

Per-file cyclomatic complexity (`scc`), comparing the old
`controller/`+`model/`+`service/`+`repository/` layer (`crud`,
now deleted outright on both `json-hypermedia` and `main`) against
the `feature/*` slices and shared kernel on each:

| | files | avg complexity | median | max |
|---|---|---|---|---|
| old layer (`crud`) | 20 | 5.8 | 0 | 113 |
| `feature/*` (`json-hypermedia`) | 49 | 2 | 2 | 17 |
| shared kernel (`json-hypermedia`) | 72 | 1.7 | 0 | 27 |
| `feature/*` (`main`) | 53 | 2.3 | 2 | 17 |
| shared kernel (`main`) | 76 | 2 | 0 | 27 |

The shared kernel's higher max is expected and correct: it holds the
`Elevator` aggregate itself (state machine, scheduling), the one
place this architecture concentrates real domain complexity rather
than smearing it across every controller that used to touch it.

## 3. Average cost of one new capability (after)

Per-slice total lines, `feature/*` (after) -- each is a fully
independent, separately testable unit:

| slice | lines |
|---|---|
| `clearobstruction` | 146 |
| `obstructdoors` | 158 |
| `opendoors` | 160 |
| `exitmaintenance` | 165 |
| `closedoors` | 165 |
| `reportload` | 170 |
| `entermaintenance` | 179 |
| `triggeremergencyrecall` | 179 |
| `selectfloor` | 207 |
| `streamevents` | 227 |
| `callelevator` | 234 |
| `insertkey` | 345 |
| `viewstatus` | 377 |
| **average** | **208.6** |

(`json-hypermedia`'s `feature/*` is already essentially this same
set of slices, since its backend finished independently -- this
table isn't repeated per-ref for that reason.)

## 4. The BFF: consolidated, then removed

`json-hypermedia` shows a step slice 8 doesn't: the BFF route count
already collapsed from one-per-verb to one-per-concern (a single
`commands.post.ts` mirroring elevator-api's own single command
endpoint) *before* it was deleted outright on `main`:

| | `crud` | `json-hypermedia` | `main` |
|---|---|---|---|
| BFF + store files | 16 | 6 | 0 |
| BFF + store lines | 493 | 656 | 0 |
| BFF route files (`server/api/**`) | 14 | 4 | 0 |
| ... their avg / median / max lines | 12.4 / 11 / 21 | 22 / 24 / 30 | -- |
| `stores/elevator.ts` lines | 229 | 461 | 0 |
| Duplication among route files | 20.2% (8) | 4.5% (1) | -- |
| Network hops per rider action | 2 | 2 | 1 |

Before/mid: browser -> BFF route -> elevator-api. After: browser
-> elevator-api directly (Caddy is a transparent reverse proxy,
not a logic hop).

Route *count* dropped 15 -> 4 between `crud` and
`json-hypermedia` -- one consolidated command proxy instead of one per
verb. But the store grew: **229 -> 461 lines**. This is not a
contradiction, it's the whole point of this middle data point: a
smart SPA client consuming a hypermedia JSON API still has to
*interpret* that hypermedia -- find the right operation by rel,
follow its `href` and method, echo its hidden fields -- which is
real logic, replacing "duplicate the business rule" with "parse
the affordance correctly". Different work, not obviously less of
it, and it still needs testing client-side (section 10). Only
removing the smart client too (`main`) gets that logic,
and its tests, out of the front end entirely -- confirming
`docs/architecture.md`'s own thesis that named commands and
hypermedia are load-bearing for each other, neither alone.

Default jscpd thresholds (min 5 lines / 50 tokens) miss most
duplication in files this small (crud's routes average
12.4 lines). Lowering the thresholds to fit surfaces the
percentages above; the routes are near-identical, differing only
in HTTP verb and path, e.g. `open-doors.post.ts` and
`close-doors.post.ts` on `crud`:

```ts
export default defineEventHandler(async (event) => {
  const id = getRouterParam(event, 'id')
  const config = useRuntimeConfig()

  return await $fetch(`${config.serviceApiUrl}/elevators/${id}/open-doors`, {
    method: 'POST'
  })
})
```

Every other route on `crud` repeats this shape, varying
only the path segment and verb -- exactly the kind of repetition
a client following hypermedia links never has to write, because
it never constructs the URL or the verb itself.

Deployable *service* count is unchanged throughout (the BFF lived
inside the same Nuxt container, not a separate one) -- the
removed cost was pure pass-through/proxy code and an extra hop,
not a deployable.

## 5. Duplication (whole-directory)

`elevator-ui` below is `app/` + `server/` where a BFF exists
(`crud`, `json-hypermedia`), `app/` alone otherwise:

| | `crud` | `json-hypermedia` | `main` |
|---|---|---|---|
| elevator-api | 7.1% (7 clones) | 18.8% (93 clones) | 16% (94 clones) |
| elevator-ui | 0% (0 clones) | 0.9% (2 clones) | 2% (2 clones) |

elevator-api's duplication is close across all three, consistent
with `json-hypermedia` and `main` sharing essentially the
same backend. Read the elevator-api numbers carefully regardless:
exact-token clone detection over many small, structurally-identical
files naturally reports a higher percentage than a few large ones
would, even when nothing is copy-pasted. Sampling its clones
confirms this: they are import blocks and same-shaped
`AffordanceContributor`/`Command` implementations (one interface,
many slices) -- not duplicated business logic. `crud`'s
clones are the opposite kind: copy-pasted controller/validation
logic between e.g. `CallController` and `CarCallController` -- the
smell this refactor targets. Percentage alone conflates the two;
only sampling the clones tells them apart.

## 6. API surface

| | `crud` | `json-hypermedia` | `main` |
|---|---|---|---|
| Endpoint mappings (`@*Mapping`) | 12 | 7 | 9 |
| Hard-coded `/elevators/...` literals in elevator-ui | 34 | 13 | 0 |

`crud`: one URL per verb (`/calls`, `/car-calls`,
`/open-doors`, `/close-doors`, `/obstruct-doors`,
`/clear-obstruction`, `/weight`, `/maintenance`, ...). From
`json-hypermedia` onward, every command funnels through the shared
`POST /elevators/{id}` on the backend -- but the hard-coded-literal
count on `json-hypermedia` shows the client still isn't off the hook:
its BFF and store still construct `/elevators/{id}` URLs rather
than following links, because there's still a smart client to do
the constructing. Only `main` gets this to zero -- the
client follows links instead of building them.

## 7. The whole diff, by application

Total, `crud` -> `main`:

| app | files changed | + | - | added | modified | deleted |
|---|---|---|---|---|---|---|
| elevator-api | 210 | 9901 | 1903 | 179 | 4 | 27 |
| elevator-ui | 34 | 1518 | 2585 | 4 | 8 | 22 |
| elevator-auth | 1 | 1 | 0 | 0 | 1 | 0 |
| docs | 12 | 17881 | 435 | 11 | 1 | 0 |
| **whole repo** | **265** | **29644** | **5030** | | | |

elevator-ui's diff is net-negative (more deleted than added) despite
unchanged feature parity -- the BFF/store deletion in section 4.

Split into its two legs either side of `json-hypermedia` (+/- only):

| app | `crud` -> `json-hypermedia` | `json-hypermedia` -> `main` |
|---|---|---|
| elevator-api | +8830/-1894 | +1468/-445 |
| elevator-ui | +995/-443 | +1513/-3132 |
| elevator-auth | +0/-0 | +1/-0 |
| docs | +4619/-435 | +13279/-17 |

The first leg is almost entirely elevator-api (the backend
migration); the second leg is almost entirely elevator-ui (BFF and
store deletion) -- the two legs of this refactor really were
separable, and `json-hypermedia` is the seam between them.

Read `docs`'s numbers with one caveat: this repo's `docs/`
also grew for reasons unrelated to the refactor itself (a talk
manuscript, this very report) -- it is not a proxy for "code
written to migrate the architecture" the way the other rows are.

## 8. Versioning cost, extrapolated

Anchor (cited in `docs/plan.html`, section 09, via Ulsberg's
*API Change Strategy*, costing by Jacques Dubray): point-to-point
versioning runs ~45% more expensive than a compatible-change
strategy at 4 concurrent versions.

We fit a linear rate from that single point: `cost(n) = 1 + k*(n-1)`,
`cost(4) = 1.45` => `k = 0.15`. Applied to 5 new versions
living alongside the original (6 concurrent
versions): `cost(6) = 1.75`,
i.e. an estimated **+75%**
maintenance cost for CRUD-style point-to-point versioning vs. a
compatible-change (hypermedia) strategy. This is a linear
extrapolation of one cited data point, not a new empirical
measurement -- treat it as illustrative, not precise.

Grounded in this repo's own measured CRUD surface (`crud`,
the versionable layer with no compatible-extension mechanism):

| | value |
|---|---|
| `controller+model+service+repository` lines | 955 |
| of which `controller` (verb layer) alone | 237 |
| its test lines | 970 |

Projected lines to maintain under three strategies for "5 new
versions live in parallel":

| strategy | lines to maintain |
|---|---|
| Scenario A -- full fork per version | ~5730 |
| Scenario B -- partial fork (controller only) | ~2140 |
| REST+DDD -- 5 new slices, additive | ~1043 |

Scenario A: the whole old layer forks per version (worst case, a
naive point-to-point strategy where behaviour differs by version).
Scenario B: only the verb/controller layer forks per version;
`model`/`service`/`repository` stay shared across versions. Both
multiply the *entire existing surface* by 6. The REST+DDD
figure is not a multiple of anything existing: 5 new capabilities
cost 5 new slices (section 3's average) and change nothing already
built or tested.

The REST+DDD side's cost of "5 more versions" is additive (new
slices), not multiplicative against the whole surface -- because
new capability is a new `rel`, never a new version of an existing
one (see `docs/architecture.md`, "No versioning").

This section's subject (the CRUD surface being versioned) is
orthogonal to `json-hypermedia`: the single consolidated command
endpoint that removes the need to version at all is a backend
property, and `json-hypermedia`'s backend already has it -- this
cost was avoided from that milestone onward, not just at the end.

## 9. Framework coupling: infrastructure moved in-house

Hypothesis: the REST+DDD side depends on far fewer framework symbols
in its domain and application code, because persistence, hypermedia
and rendering became purpose-built infrastructure at the boundary
instead of framework annotations reaching into the domain model
itself. Measured directly, by counting `import` lines from
`org.springframework.*`, `jakarta.*` and `lombok.*` (Spring's DI
stereotypes, JPA, and Lombok):

| | files | with framework imports | distinct symbols |
|---|---|---|---|
| before: `model/` (domain) | 6 | 3 | 10 |
| before: `service/` (business logic) | 1 | 1 | 3 |
| before: domain+business logic, combined | 7 | 4 | 13 |
| mid: domain+application, combined | 59 | 21 | 1 |
| after: `shared/domain` | 28 | 0 | 0 |
| after: Command+Handler+AffordanceContributor | 32 | 22 | 1 |
| after: domain+application, combined | 60 | 22 | 1 |
| whole `elevator-api/src/main`, `crud` | 24 | 17 | 30 |
| whole `elevator-api/src/main`, `json-hypermedia` | 125 | 73 | 43 |
| whole `elevator-api/src/main`, `main` | 133 | 78 | 61 |

`json-hypermedia`'s domain+application coupling already matches
`main`'s -- expected, since this is purely a backend
property and `json-hypermedia`'s backend already finished migrating.

Before: `model/Elevator.java` is itself `@Entity`-annotated --
the same class is the JPA entity, the domain model, and the JSON
response (the "Model reuse" smell). `ElevatorService` -- the
business logic -- imports `org.springframework.http.HttpStatus`
and throws `ResponseStatusException` directly: a domain refusal
is expressed as an HTTP status code inside the business logic.

After: `shared/domain` has zero framework imports across
28 files (by design -- see
`docs/architecture.md`'s slice 0: "no Spring/JPA/Lombok"). Command
and handler classes carry exactly one framework symbol each
(`@Component`, for dependency injection), never a framework
exception type or persistence annotation. A refusal is
`CommandRefused`, a plain domain type with no framework
dependency; only the controller layer decides how to render it.

### Upgrade-exposure proxy

Distinct framework symbols are a proxy for exposure to a major
framework version bump (e.g. Spring Boot's next major, or a JPA
provider swap): each one is a place a breaking change can land, and
every file importing it is a file that may need to change. The
before side's domain+business-logic layer relies on
13 distinct framework symbols across
4/7 files; the after side's domain+application
layer relies on 1 across
22/60 files. A Spring or JPA major-version migration on the
before side plausibly touches the domain model itself; on the
after side it is contained to `shared/web`, `shared/persistence`
and `config/` -- the files whose job is exactly to absorb that
kind of change -- without touching `shared/domain` or any slice's
`Command`/`Handler`. This repo has no in-history major-version
migration of elevator-api to point to directly (the one Spring Boot
v3->v4 bump in git history predates elevator-api's own code), so
this is reasoned from measured coupling, not a second empirical
data point -- treat it accordingly.

## 10. Tests: proximity, precision, speed

Where a rule is tested, and how directly, before vs after. A test
file counts as needing a Spring context if it uses
`@SpringBootTest`, `@WebMvcTest`, `@DataJpaTest` or
`@AutoConfigureMockMvc`; everything else is a plain JUnit unit
test with no framework runtime at all.

| | `crud` | `json-hypermedia` | `main` |
|---|---|---|---|
| elevator-api: unit files / methods | 0 / 0 | 26 / 134 | 28 / 143 |
| elevator-api: Spring files / methods | 8 / 47 | 17 / 62 | 18 / 68 |
| elevator-ui: e2e spec files / cases | 1 / 2 | 1 / 2 | 1 / 2 |
| ... their assertions (`expect(`) | 6 | 6 | 4 |
| elevator-ui: client unit test files / cases | 1 / 9 | 1 / 17 | 0 / 0 |
| ... their lines | 182 | 573 | 0 |

`crud`: 100% of elevator-api tests need a full Spring
context. `json-hypermedia`: 39.5%. `main`: 39.1% -- again
converged with `json-hypermedia` at the backend level, as expected.

The front end tells a different story. `json-hypermedia`'s
client-side unit test suite didn't shrink on the way from
`crud` -- it **grew**, 182 -> 573 lines
(9 -> 17 cases). `crud`'s store tested
"which requests are pending" (a duplicated business rule);
`json-hypermedia`'s tests "does the store correctly interpret the
hypermedia response" -- finding the right operation by rel,
following its `href`/method, echoing hidden fields. Different
tier-appropriate-sounding problem, same result: real logic in
the client, still needing tests, still not where it belongs.
Only `main` gets this to zero, because only there does
the client stop interpreting anything -- it renders what it's
handed.

The e2e suite is essentially unchanged in case count throughout
(it covers the same shell/interaction chrome), though its
assertion count drops on `main`
(6 -> 6 -> 4): its own comment says why -- more of the
page is now legitimately rendered by elevator-api itself and
morphed in by Datastar, so there is less static Nuxt markup left
for a shell-only smoke test to assert on.

What disappeared entirely between `json-hypermedia` and `main`
is the client-side unit test suite itself, because
`stores/elevator.ts` is gone -- there is nothing left in the
client worth unit-testing. `crud`'s own test names
said what it was really testing: `filters served calls out of
pendingCalls`, `collects pending floors from both call types` --
a business rule (which requests are still pending), requiring
five mocked HTTP endpoints and a Pinia store just to assert a
filter.

```ts
// crud: elevator-ui/test/unit/elevatorStore.test.ts
registerEndpoint('/api/key', { method: 'GET', handler: () => ... })
registerEndpoint('/api/key', { method: 'POST', handler: ... })
registerEndpoint('/api/elevators/1/status', { ... })
// ...three more registerEndpoint calls, then, finally:
it('filters served calls out of pendingCalls', () => { ... })

// json-hypermedia: same file, same effort, different problem --
// now testing hypermedia interpretation, not a business rule:
it("posts to the operation's own href and method, echoing its
    hidden type", async () => { ... })

// main: elevator-api RequestQueueTest.java -- no mocks, no
// Spring context, no HTTP layer, testing the type that owns the
// rule directly:
void twoRidersPressingTheSameLandingButtonIsOneCall() {
  RequestQueue queue = RequestQueue.empty();
  queue.addLanding(new LandingCall(new Floor(3), Direction.UP));
  boolean addedAgain = queue.addLanding(new LandingCall(new Floor(3), Direction.UP));
  assertThat(addedAgain).isFalse();
}
```

### Measured, not estimated

The actual `./gradlew test` wall-clock time for each
worktree's **elevator-api** suite, on this machine:

| | `crud` | `json-hypermedia` | `main` |
|---|---|---|---|
| Tests executed | 47 | 196 | 211 |
| Wall-clock time | 26.3s | 23.7s | 28s |
| Avg per test | 558.7ms | 120.7ms | 132.7ms |

Total wall-clock time is a noisy number to trust run-to-run --
it's dominated by fixed JVM/Gradle daemon startup, common to
both suites, more than by test logic itself, and can flip
depending on machine load. Average time per test isolates the
real difference: 4.2x lower per test on the after side, while running
4.5x more of them. That is the payoff of most tests no longer
paying Spring context startup: isolating just
`shared/domain`'s own test suite
(70 tests, no `@SpringBootTest` anywhere) runs in
2.2s -- 31.8ms/test on average, including the JVM's own
startup (this isolated run pays that cost too; a
`@SpringBootTest`-backed test pays it again per context on top
of an application context and an embedded H2 database, and (per
this codebase's test config) often a fresh context per class
rather than a shared, cached one).

### Does running the front-end tests change any of this?

Checked by hand (not automated here -- `npm run test:e2e` needs
a Nuxt dev server and a browser, plus a fresh `npm install` per
worktree, none of which this script wires up): `elevator-ui`'s
Playwright suite ran in **8.3-9.6s on `crud`**,
**7.7-8.3s on `json-hypermedia`**, and **7.4-9.2s on `main`**,
across two runs each -- indistinguishable within normal
run-to-run noise, dominated by fixed Nuxt
dev-server cold start and Chromium launch. This is expected, not
a gap in the measurement: all three spec files say in their own
comments that they deliberately never call elevator-api --
`"renders ... regardless of whether elevator-api is reachable"`.
Running the front-end tests does not strengthen the speed
argument, because they are not built to exercise the thing that
changed (the backend, and whether a BFF sits in front of it) --
and reporting a null result honestly is worth more here than
a number dressed up to look like it supports the story.

