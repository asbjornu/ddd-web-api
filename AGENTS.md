...
- Before presenting: `docker compose up` alone never rebuilds, so a branch
  ...
  script only exists on `main`; each branch's compose file defaults
  `IMAGE_TAG` to its own branch name, so building on each branch tags its
  images correctly without the script needing to pass `IMAGE_TAG`
  explicitly. During the talk, `git checkout <branch> && docker compose up`
  then only ever starts already-built images.
