...
@Transactional
class ClearObstructionControllerTest {
    ...
    @Test
    void clearingAnObstructionSucceeds() throws Exception {
        ...
        mockMvc.perform(post("/elevators/1")
                        ...
                        .content("{\"type\": \"ClearObstruction\"}"))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("\"obstructed\" : false")));
    }

    @Test
    void clearingWhenNotObstructedIsAConflict() throws Exception {
        mockMvc.perform(post("/elevators/1")
