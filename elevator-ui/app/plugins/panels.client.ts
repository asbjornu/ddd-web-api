...
// see app/plugins/shaft.client.ts's own comment for why this project
// builds bespoke markup this way instead of styling the generic
// dl/form output directly (main.css hides the generic call-elevator/
// open-doors/close-doors/obstruct-doors/clear-obstruction forms and
// the generic dl once these exist, exactly as CallPanel/StatusDisplay
// were the only rendering of those commands before).
//
...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  function floorRange(): [number, number] | null {
    const options = Array.from(
      document.querySelectorAll(`#${contentId} select[name="floor"] option`)
    )
  ...
  // Same reasoning as shaft.client.ts's own watch(): observe
  // document.body itself, since a live SSE patch or a command's own
  // response replaces #elevator-content outright, and wait for Nuxt's
  // own hydration to finish before touching the DOM at all.
  function watch() {
  ...
  nuxtApp.hook('app:mounted', watch)
})
