...
// the car -- see app/assets/css/main.css's own comment) but is built
// and kept live from data elevator-api already rendered into the DOM
// (the status dl's own currentFloor/direction/doorPosition/
...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  // Waits for Nuxt's own initial hydration to finish before touching the
  // DOM at all: starting any earlier races this plugin's own mutations
  // against Vue's hydration walk of the very same #shaft div, which Vue
  // then reports as a hydration mismatch and can spin on indefinitely.
